...
The following python modules need to be installed on your computer: obspy, matplotlib, geopy, numpy
...
STATION = "R21C3"  # Station code of local station to plot
STA_LAT = 13.7268  # Latitude of local station  
STA_LON = 100.7788  # Longitude of local station
...
TDOWNLOAD = 3600 # data length to download and write to file
...
F1 = 0.4  # High-pass filter corner for seismometers up to 90 degrees
F2 = 6.0  # Low-pass filter corner 
F3 = 0.6  # High-pass filter corner for seismometers from 90 degrees
F4 = 2.0  # Low-pass filter corner 

EXCLUDE=['RCDAD','R7A67','RB938','RE0EF','R9505','RED8C','RD8B4','RB4AC','RAA7C','R77F5','RA533','RB489','RA993','RDBE0','R9F0A','RA2B7','RBAED']
EXCLUDE+=['RDDA1','R569B','RC57A','R6A18','R9B84','S2768','R6661','RAF99','RCC6E','R09EF','R0B0A','R4FF5','R57EC','RB0E1','RD269']
EXCLUDE+=['RF808','R6C9E','RF209','RB6EF','R01F9'] # Canary island Raspberry Shakes, or not?
EXCLUDE+=['RF4C9','RC20A','RBCD3','R5791','R8504','REC8C','R376A']
EXCLUDE+=['R93B9','RE9A7','R3A99','R6CAB','RFE51','R8D78','R2EE0','R4DE2','RC79E','RCC84','R71FE','RAE44','R54E5','R8296','R2791','RC0D7','RC383','R0738','R9F7B','RCF56'] # Shadow zone
EXCLUDE+=['RC848','R87FD','RA0BA','RA533','RAA7C','R13BB','R522C','R7D6B','R359E','RA2B7','RDEE3','RA825','R923A','R1B4E','RFCBE','R9CAE','R3302','R25AC','S4458','R2683']
EXCLUDE+=['RCC68','RD2D3','RE443','R976A','RDDF8','R17FC']
        ...
        if fileflag != "N":
            try:
                ...
                if station[3] <= 50:
                    ...
                    filtertext1 = "Bandpass Filter: freqmin="+str(F1)+", freqmax="+str(F2)+" at up to 50 degrees."
                else:
                    st.filter('bandpass', freqmin=F3, freqmax=F4)
                    filtertext2 = "Bandpass Filter: freqmin="+str(F3)+", freqmax="+str(F4)+" at greater than 50 degrees."
...
# Save the plot to file, then print to screen
    plt.savefig(nospaces(FILE_STEM)+'-R21C3-Section.png')
    plt.show()
