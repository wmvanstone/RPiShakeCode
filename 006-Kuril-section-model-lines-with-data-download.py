...
Copy this file and ShakeNetwork2019.csv into the same folder
...
NETWORK_DATA = "ShakeNetwork2019.csv" # data file containing seismometer names and locations
GLOBE_PHASES = [
    # Phase, distance
    ('P', 26), 
    ('PP', 60),
...
for station in cutStations:
    distance = locations2degrees(EVT_LAT, EVT_LON, float(station[4]), float(station[5]))
    if (distance <= MAX_DIST and distance >= MIN_DIST and station[0] not in EXCLUDE):
        seismometers.append([station[0], round(float(station[4]),4), round(float(station[5]),4), round(distance,4)])
